...
export function Highlight({
  ...
  return (
    <span
      className={cn(
        'bg-brand-cream/10 p-1 py-0.5 font-bold text-brand-cream',
        className,
      )}
    >
      {children}
    ...
    >
      <div className="text-muted-foreground text-sm font-normal select-none">
        ...
        <div className="flex flex-row py-1">
          <Star className="size-4 fill-brand-cream text-brand-cream" />
          <Star className="size-4 fill-brand-cream text-brand-cream" />
          <Star className="size-4 fill-brand-cream text-brand-cream" />
          <Star className="size-4 fill-brand-cream text-brand-cream" />
          <Star className="size-4 fill-brand-cream text-brand-cream" />
      ...
      <div className="flex w-full items-center justify-start gap-5 select-none">
        <Image
          ...
          src={img || ''}
          alt={name}
          className="size-10 rounded-full ring-1 ring-brand-cream/20 ring-offset-2"
        />
        <div>
...
const testimonials = [
  ...
  },
  {
    ...
    description: (
      <p>
        I was skeptical at first, but Kareè proved me wrong.
        <Highlight>
  ...
  },
  {
    ...
    description: (
      <p>
        As a non-technical founder, Kareè has been a game-changer for our MVP.
...
export default function Testimonials() {
  return (
    <section className="relative container py-10">
      {/* Decorative elements */}
      <div className="absolute top-20 -left-20 z-10 h-64 w-64 rounded-full bg-brand-secondary/5 blur-3xl" />
      <div className="absolute -right-20 bottom-20 z-10 h-64 w-64 rounded-full bg-brand-secondary/5 blur-3xl" />
      <motion.div
        ...
        className="!mb-20"
      >
        <h2 className="text-foreground mb-4 text-center text-4xl leading-[1.2] font-bold tracking-tighter md:text-5xl font-pp-mori">
          What Our Clients Are Saying
        </h2>
        <h3 className="text-sm md:text-base text-brand-black mx-auto mb-8 max-w-lg text-center text-lg font-normal tracking-tight text-balance">
          Don&apos;t just take our word for it. Here&apos;s what{' '}
          <span className="bg-gradient-to-r from-brand-secondary to-brand-cream bg-clip-text text-transparent">
            real clients
          </span>{' '}
          are saying about{' '}
          <span className="font-semibold text-brand-cream">Kareè</span>
        </h3>
      </motion.div>
      <div className="relative mt-6 max-h-screen overflow-hidden">
        ...
        <div className="from-background pointer-events-none absolute inset-x-0 bottom-0 h-1/4 w-full bg-gradient-to-t from-[#f8f8f8]"></div>
        <div className="from-background pointer-events-none absolute inset-x-0 top-0 h-1/4 w-full bg-gradient-to-b from-[#f8f8f8]"></div>
      </div>
    </section>
  );
}